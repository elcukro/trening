...
test('Etap 5: sprzęt i wyjazd', async ({ page }) => {
  ...
  await page.reload()
  await expect(page.getByRole('button', { name: 'Cofnij odhaczenie' }).first()).toBeVisible()
  // rower użytkownika daje serwis cykliczny dobrany do hamulców
  await expect(page.getByText('Klocki i ścianki obręczy')).toHaveCount(0)
  await page.getByRole('button', { name: '+ Rower' }).click()
  await page.getByLabel('Nazwa').fill('Szosa testowa')
  await page.getByLabel('Hamulce').selectOption('rim')
  await page.getByRole('button', { name: 'Zapisz', exact: true }).click()
  await expect(page.getByText('Klocki i ścianki obręczy')).toBeVisible()
  await expect(page.getByText('Klocki i tarcze hamulcowe')).toHaveCount(0)
  await page.getByLabel('Co zrobione').fill('nowy łańcuch')
  await page.getByRole('button', { name: 'Dopisz do dziennika' }).click()
  await expect(page.getByText('nowy łańcuch')).toBeVisible()
