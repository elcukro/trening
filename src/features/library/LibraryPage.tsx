...
export function LibraryPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-3 grid grid-cols-2 gap-1 rounded-xl bg-slate-200 p-1 lg:max-w-md dark:bg-slate-700">
      ...
      {tab === 'bike' ? (
        <ul className="space-y-2 lg:grid lg:grid-cols-2 lg:gap-3 lg:space-y-0 xl:grid-cols-3">
          {workouts.map((w) => {
            ...
            return (
              <li key={w.id}>
                <Link to={`/biblioteka/trening/${w.id}`} className="block h-full rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition-colors hover:bg-sky-50 lg:p-4 dark:border-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700">
        ...
        <ul className="divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white lg:grid lg:grid-cols-2 lg:gap-3 lg:divide-y-0 lg:border-0 lg:bg-transparent xl:grid-cols-3 dark:divide-slate-700 dark:border-slate-700 dark:bg-slate-800 lg:dark:bg-transparent">
          {exercises.map((e) => (
            <li key={e.id} className="lg:rounded-2xl lg:border lg:border-slate-200 lg:bg-white lg:shadow-sm lg:transition-colors lg:hover:bg-sky-50 lg:dark:border-slate-700 lg:dark:bg-slate-800 lg:dark:hover:bg-slate-700">
              <Link to={`/biblioteka/cwiczenie/${e.id}`} className="flex min-h-12 items-center justify-between gap-2 px-4 py-2 lg:py-3">
