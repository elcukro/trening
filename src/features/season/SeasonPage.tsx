    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <ul className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
          {phases.map((p) => (
            <li key={p.id + p.from} className="flex items-center gap-2">
              <span className={`h-2.5 w-2.5 rounded-full ${PHASE_COLOR[p.id]}`} />
              <span className="font-medium">{PHASE_SHORT[p.id]}</span>
              <span className="text-slate-500">{p.count} tyg.</span>
            </li>
          ))}
        </ul>
      </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {summary.map((w) => {
              ...
              return (
                <tr key={w.week} className={`border-t border-slate-100 dark:border-slate-700 ${current ? 'bg-sky-50 dark:bg-sky-950/40' : ''}`}>
                  <td className="py-1.5 pr-2">
                    <Link to={`/tydzien/${lw.monday}`} className="flex items-center gap-1 font-semibold text-sky-600">
                  ...
                  <td className="py-1.5 pr-2">
                    {w.type === 'build' ? <span className="text-slate-500">build</span> : <Badge color={w.type === 'deload' ? 'bg-teal-600' : w.type === 'test' ? 'bg-indigo-600' : 'bg-violet-500'}>{WEEK_TYPE_LABEL[w.type as keyof typeof WEEK_TYPE_LABEL]}</Badge>}
                  </td>
                  ...
                  <td className="py-1.5 whitespace-nowrap">
                    {w.sat ?? '—'} / {w.sun ?? '—'}
                    {w.event && <div className="max-w-56 truncate text-orange-600" title={w.event}>{w.event}</div>}
                  </td>
                </tr>
              )
            })}
