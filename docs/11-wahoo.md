...
5. **Powiązanie planu z treningiem**: samo `workout[plan_id]` nie wystarcza. Odczyt treningu pokazuje wtedy `plan_id` ustawione, ale `plan_ids: []`, a licznik wyświetla „Select workout plan” i puste podsumowanie (TSS liczone jak dla jazdy bez struktury). Wysyłamy dodatkowo `workout[plan_ids][]`, a po zapisie odczytujemy trening i sprawdzamy, czy lista naprawdę nie jest pusta. Gdy jest, aplikacja zgłasza to jako błąd zamiast udawać sukces.
6. **Wysyłka** (`wahoo-push`): dla każdego dnia `POST /v1/plans` + `POST /v1/workouts` (start 6:00 czasu warszawskiego, `workout_type_id` 0 dla jazdy na zewnątrz, 61 dla Wattbike'a). Ponowna wysyłka tego samego dnia robi `PUT` istniejącego planu i treningu zamiast duplikatu – klucz to `wahoo_pushes(user_id, date)`, a `external_id` ma postać `data:trening:wersja_programu`.
7. **Tryb automatyczny**: raz dziennie, po uruchomieniu aplikacji, wysyłane jest **dziś + 6 dni**. Przełącznik w Ustawieniach. Ręcznie: „Wyślij 7 dni” albo przycisk **„Wyślij na Wahoo”** przy konkretnym dniu.
8. Na Bolcie treningi są w **Planned Workouts** po synchronizacji zegarka (Wi-Fi albo aplikacja ELEMNT).
